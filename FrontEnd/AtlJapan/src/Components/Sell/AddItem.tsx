...
function AddItemDetails() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await axios.get(`${BASE_URL}/api/BuySellRate/${id}`);
  ...
  const handleUpdate = async (values: CurrentValues) => {
    try {
      ...
      console.log("Intermediate values:", values);
      const { id, ...postData } = values;
      // await axios.put(`http://localhost:5291/api/BuySellRate/${id}`, values);
      const response = await axios.post(`${BASE_URL}/api/SellRate`, postData);
      // const data = response.data;
      console.log(JSON.stringify(values, null, 2));
      alert("Data updated successfully");
      navigate(`/DashboardSell`);
      // navigate(`/SellItem/${id}`);
    } catch (error) {
  ...
  return (
    <>
      <NavbarDashboard />
      <div>
        <h2 className="Dashboard-text">Add Sell Rate</h2>
        {currentValues && (
              ...
              <Form>
                ...
                <table>
                  ...
                  <tbody>
                    ...
                    <tr>
                      <td>
                        <label htmlFor="surrcharge">THC Origin:</label>
                      </td>
                      <td>
                        <Field
                          type="text"
